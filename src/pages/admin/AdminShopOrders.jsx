...
export default function AdminShopOrders() {
  ...
  useEffect(() => { setPageTitle('Quản lý đơn hàng shop'); }, []);
  const { toast } = useAdminToast();
  const [orders, setOrders] = useState([]);
  const [status, setStatus] = useState('');
  const load = async () => {
    try {
      setOrders(await shopApi.getAdminShopOrders({ status }));
    } catch (e) {
      toast.error(getAdminErrorMessage(e, 'Không thể tải đơn hàng.'));
    }
  };
  useEffect(() => { load(); }, []);
  const updateStatus = async (id, next) => {
    try {
      await shopApi.updateAdminShopOrderStatus(id, { status: next, note: 'Admin cập nhật trạng thái' });
      toast.success('Trạng thái đơn hàng đã được cập nhật.');
      load();
    } catch (e) {
      toast.error(getAdminErrorMessage(e, 'Không thể cập nhật trạng thái đơn hàng.'));
    }
  };
